...
- `screenshot.yml` no longer captures through `apitrace` and `glretrace`. That pipeline replayed a truncated trace and produced an image that was never on screen, which is where the noise screenshot in the README came from. `example_graphics` now writes its own PNG when `YUP_SCREENSHOT` names a path, using `Component::snapshotToImage` to render the tree on the GPU and read it back in-process. External capture cannot work headlessly: X keeps no backing store for a GL window, and the demo is not continuous, so it swaps once at startup and then idles
